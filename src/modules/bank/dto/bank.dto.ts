...
export class CreateBankDto {
    @ApiProperty({ description: 'Display name of the bank account', example: 'GTBank - Operations' })
    @IsString()
    @IsNotEmpty()
    name: string;

    @ApiProperty({ description: 'Bank account number (must be unique)', example: '0123456789' })
    @IsString()
    @IsNotEmpty()
...
export class UpdateBankDto {
    ...
    @ApiPropertyOptional({ description: 'Updated account number (must be unique)' })
    @IsString()
    @IsOptional()
    accountNumber?: string;
