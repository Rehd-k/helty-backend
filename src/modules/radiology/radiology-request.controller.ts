...
export class RadiologyRequestController {
  ...
  @ApiOperation({ summary: 'Create a radiology request (doctor orders imaging)' })
  create(@Body() dto: CreateRadiologyRequestDto) {
    return this.radiologyRequestService.create(dto);
  }

  @Get()
  @ApiOperation({ summary: 'List radiology requests with optional filters (worklist)' })
  findAll(@Query() query: ListRadiologyRequestsQueryDto) {
    return this.radiologyRequestService.findAll(query);
  }

  @Get(':id')
  @ApiOperation({ summary: 'Get one radiology request with schedule, procedure, images, report' })
  findOne(@Param('id') id: string) {
    return this.radiologyRequestService.findOne(id);
  }

  @Patch(':id')
  @ApiOperation({ summary: 'Update radiology request (e.g. cancel, status, notes)' })
