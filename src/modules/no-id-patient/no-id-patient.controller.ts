...
export class NoIdPatientController {
  ...
  @Get(':id')
  @ApiOperation({
    summary: 'Get a single NoIdPatient with linked services & transactions',
  })
  ...
  @Patch(':id')
  @ApiOperation({
    summary: 'Update meta fields (name, age, gender) of a NoIdPatient',
  })
  @ApiParam({ name: 'id', description: 'NoIdPatient UUID' })
  ...
  @ApiOperation({
    ...
    description:
      'Associates an already-created Service record with this NoIdPatient.',
  })
  @ApiParam({ name: 'id', description: 'NoIdPatient UUID' })
  @ApiParam({ name: 'serviceId', description: 'Service UUID' })
  addService(@Param('id') id: string, @Param('serviceId') serviceId: string) {
